...
def count_tensor_elements(x: Tensor) -> int:
    ...
    # Replace "pass" statement with your code
    rank=x.dim()
    shapelist=list(x.shape)
    num_elements=1
    for i in range(rank):
        num_elements*=shapelist[i]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return num_elements
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y=x.clone()
    idx0=torch.arange(y.shape[0]-1,-1,-1)
    y=y[idx0,:]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    M,N=x.shape
    mu=x.sum(dim=0)/M
    sig=torch.sqrt(torch.sum((x-mu)**2,dim=0)/(M-1))
    y=(x-mu)/sig
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
