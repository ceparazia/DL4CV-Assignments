...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    train_vec=x_train.reshape(num_train,-1)
    test_vec=x_test.reshape(num_test,-1)
    for i in range(num_train):
        for j in range(num_test):
            vec=(train_vec[i]-test_vec[j])**2
            dists[i,j]=torch.sum(vec)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    test_vec=x_test.reshape(num_test,-1)
    for i in range(num_train):
        dists[i]=torch.sum((train_vec[i]-test_vec)**2,dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    a=(train*train).sum(dim=1)
    b=(test*test).sum(dim=1)
    c=train.mm(test.t())
    a=a.reshape(num_train,1)
    b=b.reshape(1,num_test)
    dists=a+b-2*c
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        sample=dists[:,i]  #(num_train,)
        knndist=torch.argsort(sample)[:k]
        labels=y_train[knndist]
        uni,counts=torch.unique(labels,return_counts=True)
        y_pred[i]=uni[counts==counts.max()].min()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        num_test=x_test.shape[0]
        train_flat=self.x_train.reshape(num_train,-1)  #(num_train,D)
        test_flat=x_test.reshape(num_test,-1)  #(num_test,D)
        
        dists=torch.zeros(num_train,num_test)
        a=(train_flat*train_flat).sum(dim=1).reshape(num_train,1)
        b=(test_flat*test_flat).sum(dim=1).reshape(1,num_test)
        c=train_flat.mm(test_flat.t())
        dists=a+b-2*c  #(500,250)
        y_test_pred=torch.zeros(num_test,dtype=torch.int64)
        for i in range(num_test):
            sample=dists[:,i]   #(500,)
            sorted_sample=torch.argsort(sample)[:k]
            labels=self.y_train[sorted_sample]
            uni,counts=torch.unique(labels,return_counts=True)
            y_test_pred[i]=uni[counts==counts.max()].min()
        ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
        return y_test_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            x_train=torch.cat([x_train_folds[j] for j in range(num_folds) if j!=i],dim=0)
            y_train=torch.cat([y_train_folds[j] for j in range(num_folds) if j!=i],dim=0)
            
            x_vali=x_train_folds[i]
            y_vali=y_train_folds[i]

            classifier=KnnClassifier(x_train,y_train)
            accuracy=classifier.check_accuracy(x_vali,y_vali,k)
