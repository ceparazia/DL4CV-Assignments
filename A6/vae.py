...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15):
        ...
        self.hidden_dim = 128  #这是我自己设置的
        self.encoder=nn.Sequential(
            nn.Flatten(),  #默认从dim=1开始把后面所有维度展平  (N, 1, H, W) -> (N,H*W)
        ...
        self.decoder=nn.Sequential(
            ...
            nn.Sigmoid(),
            nn.Unflatten(dim=1,unflattened_size=(1,28,28))  #形状来自MNIST的图片分辨率28*28
        )
        ###########################################################################
        #                                      END OF YOUR CODE                   #
        ###########################################################################
...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        # Replace "pass" statement with your code
        N=x.shape[0]
        x_flatten=x.reshape(N,-1)  #(N,H*W)
        x_cond=torch.cat((x_flatten,c),dim=1)  #(N,H*W+C)
        tmp=self.encoder(x_cond)
        mu=self.mu_layer(tmp)
        logvar=self.logvar_layer(tmp)
        z=reparametrize(mu,logvar)  #(N,Z)
        z_cond=torch.cat((z,c),dim=1)  #(N,Z+C)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    # Replace "pass" statement with your code
    N=x.shape[0]
    reconst_loss=F.binary_cross_entropy(x_hat,x,reduction="sum")/N  #注意x和x_hat的顺序不能反。F.binary_cross_entropy要把ground truth放在第二个参数
    KL_loss=-0.5*torch.sum(1+logvar-mu*mu-logvar.exp())/N
