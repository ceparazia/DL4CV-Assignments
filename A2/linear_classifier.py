...
def svm_loss_vectorized(
    ...
    num_train=X.shape[0]
    S=X.mm(W)  #(N,C)
    correct_scores=S[torch.arange(num_train),y]   #(N,)
    margins=S-correct_scores.reshape(num_train,1)+1
    margins[torch.arange(num_train),y]=0.0
    loss=margins.clamp(min=0).sum()
    loss/=num_train
    loss+=reg*torch.sum(W*W)
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    l=torch.randint(0,X.shape[0],(batch_size,))
    X_batch=X[l]
    y_batch=y[l]
    #########################################################################
    #                       END OF YOUR CODE                                #
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [0.9e-2,1e-2,1.1e-2]
    regularization_strengths = [0.8e-2,0.9e-2,1e-2,1.1e-2,1.2e-2]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        s=W.t().mv(X[i])   #s:(C,)
        s=s-s.max()  #保证最大的x是0，防止e^x过大，产生计算不稳定
        s=torch.exp(s)
        s=s/(s.sum())
        loss+=-torch.log(s[y[i]])
        for j in range(num_classes):
            if j==y[i]:
                dW[:,j]+=(s[j]-1)*X[i]
            else:
...
def softmax_loss_vectorized(
    ...
    num_classes=W.shape[1]
    S=X.mm(W)  #(N,C)
    maxvec,_=S.max(dim=1)
    S=S-maxvec.reshape(-1,1)
    S=S.exp()
    SUM=S.sum(dim=1)  #(N,)
    S=S/(SUM.reshape(-1,1))
    correctlist=S[torch.arange(num_train),y]
    loss=(-torch.log(correctlist)).mean()
