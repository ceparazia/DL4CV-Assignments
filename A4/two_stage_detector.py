...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        input_C=in_channels
        for output_C in stem_channels:
            conv1=nn.Conv2d(input_C,output_C,kernel_size=3,stride=1,padding=1)
            nn.init.normal_(conv1.weight,mean=0,std=0.01)
            nn.init.zeros_(conv1.bias)
            stem_rpn.append(conv1)
        ...
        # Replace "pass" statement with your code
        final_C=stem_channels[-1]
        conv1=nn.Conv2d(final_C,num_anchors,kernel_size=1,stride=1,padding=0)
        nn.init.normal_(conv1.weight,mean=0,std=0.01)
        nn.init.zeros_(conv1.bias)
        self.pred_obj=conv1    

        conv2=nn.Conv2d(final_C,4*num_anchors,kernel_size=1,stride=1,padding=0)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # Replace "pass" statement with your code
        for pname,p in feats_per_fpn_level.items():
            B,_,_,_=p.shape   #(B,fpn_C,H,W)
            stem_res=self.stem_rpn(p)  #(B,final_C,H,W)
            obj_pred_res=self.pred_obj(stem_res)  #(B,num_anchors,H,W)
            box_pred_res=self.pred_box(stem_res)  #(B,4*num_anchors,H,W)
            object_logits[pname]=obj_pred_res.permute(0,2,3,1).reshape(B,-1)  #(B,H * W * num_anchors)
            boxreg_deltas[pname]=box_pred_res.permute(0,2,3,1).reshape(B,-1,4) #(batch_size, H * W * num_anchors, 4)
...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            box_w=math.sqrt(area/aspect_ratio)
            box_h=area/box_w

            l=locations[:,0]-box_w/2   #(H*W,)
            t=locations[:,1]-box_h/2
            r=locations[:,0]+box_w/2
            b=locations[:,1]+box_h/2

            anchor_boxes.append(torch.stack([l,t,r,b],dim=-1))  #每一个是(H*W,4)
            ##################################################################
            #                           END OF YOUR CODE                     #
            ##################################################################
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    l1,l2=torch.meshgrid(boxes1[:,0],boxes2[:,0])  #2个(M,N)
    t1,t2=torch.meshgrid(boxes1[:,1],boxes2[:,1])
    r1,r2=torch.meshgrid(boxes1[:,2],boxes2[:,2])
    b1,b2=torch.meshgrid(boxes1[:,3],boxes2[:,3])

    inter_lr=(torch.min(r1,r2)-torch.max(l1,l2)).clamp(min=0) #(M,N)
    inter_tb=(torch.min(b1,b2)-torch.max(t1,t2)).clamp(min=0)
    inter=inter_lr*inter_tb  #(M,N)
    union=(r1-l1)*(b1-t1)+(r2-l2)*(b2-t2)-inter  #(M,N)
    iou=inter/union  #(M,N)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rcnn_get_deltas_from_anchors(
    ...
    deltas = None
    # Replace "pass" statement with your code
    bg_mask=(gt_boxes[:,:4]==-1)  #(N,4)
    neu_mask=(gt_boxes[:,:4]==-1e8)  #(N,4)
    ...
    gt_x=(gt_boxes[:,0]+gt_boxes[:,2])/2
    gt_y=(gt_boxes[:,1]+gt_boxes[:,3])/2
...
def rcnn_apply_deltas_to_anchors(
    ...
    dx,dy,dw,dh=deltas.unbind(dim=1)  #得到4个(N,)
    ...
    output_boxes=torch.stack([out_l,out_t,out_r,out_b],dim=-1)  #(N,4)
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        shape_per_fpn_level={}
        for pname,p in feats_per_fpn_level.items():
            shape_per_fpn_level[pname]=p.shape
        device=feats_per_fpn_level['p3'].device  #随便取一个tensor,识别device
        ...
        for image in range(B):
            gt_boxes_this_im=gt_boxes[image]  #(M,5)
            mat_gt_boxes_this_im=rcnn_match_anchors_to_gt(anchor_boxes,gt_boxes_this_im,iou_thresholds=self.anchor_iou_thresholds)
            # N=3个平面所有的HWA之和
            #mat_gt_boxes_this_im 是(N,5)  表示所有N个锚框匹配到的GTBOX
            matched_gt_boxes.append(mat_gt_boxes_this_im)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        if self.training:
            ...
            # Replace "pass" statement with your code
            #⭐这个num_samples设置为num_images*self.batch_size_per_image
            fg_idx,bg_idx=sample_rpn_training(matched_gt_boxes,
                                              num_samples=num_images*self.batch_size_per_image,
                                              fg_fraction=0.5)
            idx=torch.cat([fg_idx,bg_idx],dim=0)
            matched_gt_boxes = matched_gt_boxes[idx]  #(-1,5)
            pred_obj_logits = pred_obj_logits[idx]  #(-1,)
            pred_boxreg_deltas = pred_boxreg_deltas[idx]  #(-1,4)
            anchor_boxes=anchor_boxes[idx]

            gt_objectness=(matched_gt_boxes[:,4]!=-1).float()  #(-1,)
            loss_obj = F.binary_cross_entropy_with_logits(pred_obj_logits, gt_objectness, reduction="none")


            gt_deltas=rcnn_get_deltas_from_anchors(anchor_boxes,matched_gt_boxes[:,:4])
    ...
    def predict_proposals(
        self,
        anchors_per_fpn_level: Dict[str, torch.Tensor],  #anchors_per_fpn_level['p3']是(HWA,4)
        pred_obj_logits: Dict[str, torch.Tensor],  #pred_obj_logits['p3']是(B,HWA)
        pred_boxreg_deltas: Dict[str, torch.Tensor],  #pred_boxreg_deltas['p3']是(B,HWA,4)
        ...
        for level_name in anchors_per_fpn_level.keys():
            ...
            level_anchors = anchors_per_fpn_level[level_name]  #(HWA,4)

            # shape: (batch_size, HWA), (batch_size, HWA, 4)
            level_obj_logits = pred_obj_logits[level_name]  #(B,HWA)
            level_boxreg_deltas = pred_boxreg_deltas[level_name]  #(B,HWA,4)
            ...
            for _batch_idx in range(level_obj_logits.shape[0]):
                ...
                ##############################################################
                # Replace "pass" statement with your code
                delta_thisimage=level_boxreg_deltas[_batch_idx]  #(HWA,4)
                obj_logits_thisimage=level_obj_logits[_batch_idx]  #(HWA,)

                proposal_boxes=rcnn_apply_deltas_to_anchors(delta_thisimage,level_anchors)
                #(N,4)
                proposal_boxes[:,0]=proposal_boxes[:,0].clamp(min=0,max=image_size[0])
                proposal_boxes[:,2]=proposal_boxes[:,2].clamp(min=0,max=image_size[0])
                proposal_boxes[:,1]=proposal_boxes[:,1].clamp(min=0,max=image_size[1])
                proposal_boxes[:,3]=proposal_boxes[:,3].clamp(min=0,max=image_size[1])

                k1=min(self.pre_nms_topk,len(obj_logits_thisimage))
                _,topidx=obj_logits_thisimage.topk(k=k1)  #(K,)
                proposal_boxes=proposal_boxes[topidx]  #(K,4)
                obj_logits_thisimage=obj_logits_thisimage[topidx]  #(K,)

                keep=nms(proposal_boxes,obj_logits_thisimage,iou_threshold=self.nms_thresh)
                proposal_boxes=proposal_boxes[keep]  #(K',4)
                obj_logits_thisimage=obj_logits_thisimage[keep]  #(K',)

                k2=min(self.post_nms_topk,len(obj_logits_thisimage))
                _,topidx=obj_logits_thisimage.topk(k=k2)  #(K'',)
                proposal_boxes=proposal_boxes[topidx]  #(K'',4)
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        cls_pred = []
        # Replace "pass" statement with your code
        input_C=self.backbone.out_channels
        for output_C in stem_channels:
            conv1=nn.Conv2d(input_C,output_C,kernel_size=3,stride=1,padding=1)
            nn.init.normal_(conv1.weight,mean=0,std=0.01)
            nn.init.zeros_(conv1.bias)
            cls_pred.append(conv1)
        ...
        ######################################################################
        # Replace "pass" statement with your code
        cls_pred.append(nn.Flatten())  #(B,F)

        F=stem_channels[-1]*self.roi_size[0]*self.roi_size[1]
        linear=nn.Linear(F,num_classes+1)
    ...
    def forward(
        ...
        for level_name in feats_per_fpn_level.keys():
            ...
            ##################################################################
            level_feats = feats_per_fpn_level[level_name]   #(B,_,H,W)
            level_props = output_dict["proposals"][level_name]  
            #level_props是一个列表。level_props[i]是第i张图片的region_proposal (R,4)
            level_stride = self.backbone.fpn_strides[level_name]

            # Replace "pass" statement with your code
            all_rois=[]   #⭐这里的操作会把所有image的roi合起来，消去B这个维度
            #需要从若干(R,4)得到(R,5)，记录每个选区是来自哪张图片
            # 其中第二个维度要从(x1,y1,x2,y2)变成(batch_idx,x1,y1,x2,y2)
            for image in range(num_images):
                prop_this_im=level_props[image]  #(R,4)
                R=len(prop_this_im)
                if R>0:
                    batch_idx=torch.full((R,1),image,device=prop_this_im.device)   #(R,1)
                    rois_this_im=torch.cat([batch_idx,prop_this_im],dim=1)  #(R,5)
                    all_rois.append(rois_this_im)
            
            rois=torch.cat(all_rois,dim=0)  #(total_R,5)
            roi_feats=torchvision.ops.roi_align(level_feats,
                                                rois,
                                                output_size=self.roi_size,
                                                spatial_scale=1.0/level_stride,   #原图上的1步对应特征图上的1.0/level_stride步
                                                aligned=True)
        ...
        for _idx in range(len(gt_boxes)):
            ...
            mat_gt_boxes_this_im=rcnn_match_anchors_to_gt(proposals_per_image,gt_boxes_per_image,iou_thresholds=(0.5,0.5))
            matched_gt_boxes.append(mat_gt_boxes_this_im)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        matched_gt_boxes = matched_gt_boxes[idx]  #(-1,5)
        pred_cls_logits = pred_cls_logits[idx]  #(-1,num_classes)


        gt_cls=(matched_gt_boxes[:,4]+1).long()   #背景-1变成0，前景20类分到1-20。转化成整型
        loss_cls=F.cross_entropy(pred_cls_logits,gt_cls,reduction="mean")
    ...
    def inference(
        ...
        # Replace "pass" statement with your code
        prob=F.softmax(pred_cls_logits,dim=1)  #(total_R,num_classes+1)
        max_scores,max_cls=prob.max(dim=1)  #(total_R)
        keepmask=(max_scores>test_score_thresh)&(max_cls!=0)

        pred_boxes=pred_boxes[keepmask]
        pred_classes=max_cls[keepmask]-1   #确保20个前景类别对应0-19
        pred_scores=max_scores[keepmask]
