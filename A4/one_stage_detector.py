...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for output_C in stem_channels:
            ...
            conv2=nn.Conv2d(input_C,output_C,kernel_size=3,stride=1,padding=1)
        ...
        conv1=nn.Conv2d(final_C,num_classes,kernel_size=3,stride=1,padding=1)
        nn.init.normal_(conv1.weight,mean=0,std=0.01)
        # nn.init.zeros_(conv1.bias)  后面有单独设置
        self.pred_cls=conv1

        conv2=nn.Conv2d(final_C,4,kernel_size=3,stride=1,padding=1)
        nn.init.normal_(conv2.weight,mean=0,std=0.01)
        nn.init.zeros_(conv2.bias)
        self.pred_box=conv2

        conv3=nn.Conv2d(final_C,1,kernel_size=3,stride=1,padding=1)
        nn.init.normal_(conv3.weight,mean=0,std=0.01)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # Replace "pass" statement with your code
        p3=feats_per_fpn_level['p3']
        p4=feats_per_fpn_level['p4']
        p5=feats_per_fpn_level['p5']

        for pname,p in feats_per_fpn_level.items():
            stem_cls_res=self.stem_cls(p)
            class_logits[pname]=self.pred_cls(stem_cls_res).permute(0,2,3,1).flatten(1,2)  #需要转换成(batch_size, H * W, num_classes)

            stem_box_res=self.stem_box(p)
            boxreg_deltas[pname]=self.pred_box(stem_box_res).permute(0,2,3,1).flatten(1,2)
            centerness_logits[pname]=self.pred_ctr(stem_box_res).permute(0,2,3,1).flatten(1,2)
...
def fcos_match_locations_to_gt(
    ...
    for level_name, centers in locations_per_fpn_level.items():
        ...
        #centers (N,2)     .unsqueeze(dim=2) (N,2,1)       .unbind(dim=1)变成2个(N,1)
        x0, y0, x1, y1 = gt_boxes[:, :4].unsqueeze(dim=0).unbind(dim=2)
        #gt_boxes[:, :4]  (M,4)    .unsqueeze(dim=0).unbind(dim=2)变成4个(1,M)
        pairwise_dist = torch.stack([x - x0, y - y0, x1 - x, y1 - y], dim=2)
        #x - x0  (N,M)     torch.stack([...], dim=2)   (N,M,4)

        # Pairwise distance between every feature center and GT box edges:
        # shape: (num_gt_boxes, num_centers_this_level, 4)
        pairwise_dist = pairwise_dist.permute(1, 0, 2)  #(M,N,4)
        ...
        matched_idxs[match_quality < 1e-5] = -1  #没有匹配到任何GTBOX的点会记为-1，代表background
...
def fcos_get_deltas_from_locations(
    ...
    R=(x2-xc)/stride
    B=(y2-yc)/stride
    deltas=torch.stack([L,T,R,B],dim=-1)  #(N,4)

    bg_mask=(gt_boxes[:,0]==-1)  #(N,)
    deltas[bg_mask]=-1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = None
    # Replace "pass" statement with your code
    bg_mask=(deltas[:,0]==-1)  #(N)
    L,T,R,B=deltas.unbind(dim=1)  #拆成4个(N,)
    lr=torch.stack([L,R],dim=-1)  #(N,2)
    tb=torch.stack([T,B],dim=-1)
    lrmin,_=lr.min(dim=1)
    lrmax,_=lr.max(dim=1)
    tbmin,_=tb.min(dim=1)
    tbmax,_=tb.max(dim=1)
    centerness=(lrmin/lrmax*tbmin/tbmax).sqrt()
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        fpn=self.backbone(images) #接受images (B,3,H,W)，得到一个字典，其中values是p3,p4,p5    
        pred_cls_logits, pred_boxreg_deltas, pred_ctr_logits=self.pred_net(fpn)  #接受字典fpn，返回各个p对应的3个预测值（都是字典）
        ...
        for image in range(B):
            gt_boxes_this_image=gt_boxes[image]   #(M,5)
            matched_gt_boxes_this_image=fcos_match_locations_to_gt(locations_per_fpn_level,strides_per_fpn_level,gt_boxes_this_image)
            #matched_gt_boxes_this_image是一个字典
            #matched_gt_boxes_this_image['p3']是(N,5)  表示p3平面内的N个点对应的GTBOX
            matched_gt_boxes.append(matched_gt_boxes_this_image)
        ...
        # Replace "pass" statement with your code
        for image in range(B):
            matched_gt_boxes_this_image=matched_gt_boxes[image]
            matched_gt_deltas_this_image={}
            for pname in strides_per_fpn_level:
                locations=locations_per_fpn_level[pname]
                gt_boxes=matched_gt_boxes_this_image[pname]
                stride=strides_per_fpn_level[pname]
                matched_gt_deltas_this_image[pname]=fcos_get_deltas_from_locations(locations,gt_boxes,stride)
            matched_gt_deltas.append(matched_gt_deltas_this_image)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        #把p3,p4,p5上面的所有点合并到一个维度
        #3个p平面总的点数是num_locations_across_fpn_levels，记为N_total
        matched_gt_boxes = self._cat_across_fpn_levels(matched_gt_boxes) #(B,N_total,5)
        matched_gt_deltas = self._cat_across_fpn_levels(matched_gt_deltas) #(B,N_total,4)
        pred_cls_logits = self._cat_across_fpn_levels(pred_cls_logits)
        pred_boxreg_deltas = self._cat_across_fpn_levels(pred_boxreg_deltas)
        pred_ctr_logits = self._cat_across_fpn_levels(pred_ctr_logits)
        #上面所有tensor的形状是(B,N_total,...)
        ...
        # Replace "pass" statement with your code
        B,N_total,num_classes=pred_cls_logits.shape
        gt_classes=torch.zeros_like(pred_cls_logits)  #(B,N_total,num_classes)
        # for image in range(B):
        #     for dot in range(N_total):
        #         gtbox=matched_gt_boxes[B,dot]  #(5,)
        #         if(gtbox[4]!=-1):
        #             gt_classes[B,dot,gtbox[4]]=1   转化成向量形式


        cls_labels=matched_gt_boxes[:,:,4]  #(B,N_total)
        fg_mask=(cls_labels!=-1)  #BOOL型，(B,N_total),记录这个点是否是foreground

        fg_image_list,fg_dot_list=fg_mask.nonzero(as_tuple=True)
        #假设fg_mask[i,j]!=0，那么fg_image_list中有i，fg_dot_list中有j
        #设fg_mask中一共有k个非0的值，那么fg_image_list是(k,)，fg_dot_list是(k,)
        fg_cls_list=cls_labels[fg_mask].long()  #long()是把浮点型转化成长整型，确保索引为整数
        gt_classes[fg_image_list,fg_dot_list,fg_cls_list]=1

        matched_gt_deltas_unbatched=matched_gt_deltas.reshape(-1,4)  #(B,N_total,4)  ->  (B*N_total,4)
        matched_gt_centerness=fcos_make_centerness_targets(matched_gt_deltas_unbatched).reshape(B,N_total,1)


        loss_cls = sigmoid_focal_loss(inputs=pred_cls_logits, targets=gt_classes)
        
        loss_box = 0.25 * F.l1_loss(input=pred_boxreg_deltas, target=matched_gt_deltas, reduction="none")
        loss_box[matched_gt_deltas < 0] *= 0.0

        loss_ctr = F.binary_cross_entropy_with_logits(input=pred_ctr_logits, target=matched_gt_centerness, reduction="none")
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_cls_logits = pred_cls_logits[level_name][0]  #(N,num_classes)
            ...
            # Step 1:
            # Replace "pass" statement with your code
            most_confident_cls_score,most_confident_cls_idx=level_pred_scores.max(dim=1) #(N,)

            # Step 2:
            # Replace "pass" statement with your code
            keepmask=(most_confident_cls_score>test_score_thresh)  #布尔掩码(N,)

            level_cls_logits = level_cls_logits[keepmask]  #(K,num_classes)
            level_locations=level_locations[keepmask]  #(K,2)
            level_deltas = level_deltas[keepmask] #(K,4)
            level_ctr_logits = level_ctr_logits[keepmask] #(K,1)

            level_pred_classes=most_confident_cls_idx[keepmask]  #(K,)
            level_pred_scores=most_confident_cls_score[keepmask]  #(K,)
            ...
            level_pred_boxes[:,0]=level_pred_boxes[:,0].clamp(min=0,max=H)
            level_pred_boxes[:,2]=level_pred_boxes[:,2].clamp(min=0,max=H)
            level_pred_boxes[:,1]=level_pred_boxes[:,1].clamp(min=0,max=W)
            level_pred_boxes[:,3]=level_pred_boxes[:,3].clamp(min=0,max=W)
