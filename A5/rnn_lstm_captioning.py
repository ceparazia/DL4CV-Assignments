...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    tmp=x@Wx+prev_h@Wh+b
    next_h=torch.tanh(tmp)
    cache=(x, prev_h, Wx, Wh, b, tmp, next_h)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x, prev_h, Wx, Wh, b, tmp, next_h=cache
    dtmp=dnext_h*(1-next_h*next_h)  # tanh的反向传播   #(N,H)
    db=dtmp.sum(dim=0)  #(H,)
    dx=dtmp@(Wx.t())
    dprev_h=dtmp@(Wh.t())
    dWx=x.t()@dtmp
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h=h0
    for i in range(T):
        next_h,newcache=rnn_step_forward(x[:,i,:],prev_h,Wx,Wh,b)  #(N,H)
        hlist.append(next_h)
        cachelist.append(newcache)
        prev_h=next_h
    h=torch.stack(hlist,dim=1)  #(N,T,H)
...
def rnn_backward(dh, cache):
    ...
    N,T,H=dh.shape
    dxlist=[]
    dnext_h=torch.zeros_like(dh[:,0,:])  #(N,D)

    for i in range(T-1,-1,-1):
        dh_t=dh[:,i,:]+dnext_h
        #loss是h1,h2,...hT的函数
        #dh存储的是将这些hi看作独立变量时计算的梯度
        #然而hi是由h{i-1}得到的，还需要考虑时间步间的依赖关系
        #dh[:,i,:]+dnext_t才是损失函数对第t时间步的完整梯度
        #dh_t  (N,D)
        cache_t=cache[i]
        dx_t, dprev_h, dWx_t, dWh_t, db_t=rnn_step_backward(dh_t,cache_t)
        #dx_t(N, D)   dprev_h(N, H)  dWx_t(D, H)   dWh_t(H, H)   db_t(H,)
        if i==T-1:
            dWx=dWx_t
            dWh=dWh_t
            db=db_t
            ...
            db+=db_t
        dxlist.append(dx_t)
        dnext_h=dprev_h  #这是右边时间步传回左边时间步的梯度

    dxlist=dxlist[::-1]
    dx=torch.stack(dxlist,dim=1)  #(N,T,D)
    dh0=dnext_h
...
class RNN(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int):
        ...
        # Register parameters
        self.Wx = nn.Parameter(    #Parameter 张量 - PyTorch 会自动计算梯度并在训练中更新
            torch.randn(input_dim, hidden_dim).div(math.sqrt(input_dim)) #这是Xavier初始化
        )
...
class WordEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        # 使用one_hot  但是有点低效，而且设备不匹配
        # V=self.W_embed.shape[0]
        # y=F.one_hot(x,num_classes=V)  #(N,T,V)
        # #当x[i,j]==k时，y[i,j,k]=1；否则y[i,j,k]=0
        # out=y@self.W_embed  #(N,T,D)
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.im2feat=ImageEncoder(pretrained=image_encoder_pretrained,verbose=False)   #用于从图片提取特征值，得到h0
        self.featpool=nn.AdaptiveAvgPool2d((1,1))  #这是一个平均池化层，保证输出的长度、宽度是1,1
        #可以把(N,out_channels, 112/32, 112/32)池化成(N,out_channels,1,1)
        self.feat2h0=nn.Linear(input_dim,hidden_dim)  #这是线性层，可以把展平的特征值(N,D)变成h0(N,H)
        self.hidden_dim=hidden_dim
        self.wordembed=WordEmbedding(vocab_size,wordvec_dim)  #这是把(N,T)映射为(N,T,W)，每个单词被映射成一个长度为W的特征向量
        
        if cell_type=="rnn":
            self.cellmodel=RNN(wordvec_dim,hidden_dim)
        elif cell_type=="lstm":
            self.cellmodel=LSTM(wordvec_dim,hidden_dim)
        else:
            self.cellmodel=AttentionLSTM(wordvec_dim,hidden_dim)
            # self.produceA=nn.Linear(input_dim,hidden_dim)   #(N,D,4,4) -> (N,H,4,4)
            self.produceA=nn.Conv2d(input_dim,hidden_dim,kernel_size=1)

        self.h2wordscore=nn.Linear(hidden_dim,vocab_size)
    ...
    def forward(self, images, captions):
        ...
        Returns:
            loss: A scalar loss
        """
        # 下面这段话有点绕，结合PPT中的<start> man in straw → man in straw <end>例子理解
        ...
        ######################################################################
        # Replace "pass" statement with your code
        feat=self.im2feat(images) 
        # (N, 3, 112, 112) -> (N,out_channels, 112/32, 112/32)即(N,D,4,4)


        input_vec=self.wordembed(captions_in)  # (N,T) -> (N,T,W)
        
        if self.cell_type!="attn":
            feat_pooled=self.featpool(feat) #(N,out_channels, 112/32, 112/32) -> (N,out_channels,1,1)
            feat_flatten=feat_pooled.reshape(feat_pooled.shape[0],-1)  #(N,out_channels)即(N,input_dim)即(N,D)
            h0=self.feat2h0(feat_flatten)  #(N,D) -> (N,H) 
            h=self.cellmodel(input_vec,h0)  # (N,T,W) -> (N,T,H)
        else:
            # feat_1=feat.permute(0,2,3,1)  #(N,4,4,D)
            # A=self.produceA(feat_1)  #(N,4,4,H)
            # A=A.permute(0,3,1,2)  #(N,H,4,4)
            A=self.produceA(feat)
            h=self.cellmodel(input_vec,A)
            
        logits=self.h2wordscore(h)  # (N,T,H) -> (N,T,V)
        loss=temporal_softmax_loss(logits,captions_out,ignore_index=self.ignore_index)
    ...
    def sample(self, images, max_length=15):
        ...
        #######################################################################
        # Replace "pass" statement with your code
        feat=self.im2feat(images)  # (N, 3, 112, 112) -> (N,out_channels, 112/32, 112/32)
        if self.cell_type!="attn":
            feat_pooled=self.featpool(feat) #(N,out_channels, 112/32, 112/32) -> (N,out_channels,1,1)
            feat_flatten=feat_pooled.reshape(feat_pooled.shape[0],-1)  #(N,out_channels)即(N,input_dim)即(N,D)
            h0=self.feat2h0(feat_flatten)  #(N,D) -> (N,H)
        ...
        startvec=self._start*torch.ones((N,),dtype=torch.long,device=images.device)   #(N,)  值全为self._start
        x=self.wordembed(startvec)  #(N,)->(N,W)  值全为self._start对应的特征向量
        


        if self.cell_type=="rnn":
            for t in range(max_length):
                next_h=self.cellmodel.step_forward(x,prev_h)   #(N,W) -> (N,H)
                y=self.h2wordscore(next_h)  # (N,H) -> (N,V)
                _,output_idx=y.max(dim=1)  #(N,)
                captions[:,t]=output_idx   #(N,)是这一时间步输出的预测单词的索引
                x=self.wordembed(output_idx)  #(N,W)下一个时间步的输入
                prev_h=next_h
            ...
            prev_c=c0
            for t in range(max_length):
                next_h,next_c=self.cellmodel.step_forward(x,prev_h,prev_c)   #(N,W) -> (N,H)
                y=self.h2wordscore(next_h)  # (N,H) -> (N,V)
                _,output_idx=y.max(dim=1)  #(N,)
                captions[:,t]=output_idx   #(N,)是这一时间步输出的预测单词的索引
                x=self.wordembed(output_idx)  #(N,W)下一个时间步的输入
            ...
            prev_h=h0=A.mean(dim=(2, 3))
            prev_c=c0=A.mean(dim=(2, 3))
            for t in range(max_length):
                attn,attn_weights=dot_product_attention(prev_h,A)  #(N,H),(N,H,4,4)
                next_h,next_c=self.cellmodel.step_forward(x,prev_h,prev_c,attn)   #(N,W) -> (N,H)
                y=self.h2wordscore(next_h)  # (N,H) -> (N,V)
                _,output_idx=y.max(dim=1)  #(N,)
                captions[:,t]=output_idx   #(N,)是这一时间步输出的预测单词的索引
...
class LSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        hlist=[]
        for t in range(T):
            next_h, next_c=self.step_forward(x[:,t,:],prev_h,prev_c)  #(N,H)
            hlist.append(next_h)
            prev_c=next_c
            prev_h=next_h
        hn=torch.stack(hlist,dim=1)  #(N,T,H)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for t in range(T):
            ...
            next_h,next_c=self.step_forward(x[:,t,:],prev_h,prev_c,attn)  #(N,H)
            hlist.append(next_h)
            prev_h=next_h
            prev_c=next_c
        hn=torch.stack(hlist,dim=1)  #(N,T,H)
