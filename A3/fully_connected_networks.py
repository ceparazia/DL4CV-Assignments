...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        W1=weight_scale*torch.randn(input_dim,hidden_dim,dtype=dtype)
        W2=weight_scale*torch.randn(hidden_dim,num_classes,dtype=dtype)
        b1=torch.zeros(hidden_dim,dtype=dtype)
        b2=torch.zeros(num_classes,dtype=dtype)
        #这里不要忘了dtype=dtype,device=device，否则后面创建solver的时候会出错
        self.params['W1']=W1
        self.params['W2']=W2
        self.params['b1']=b1
        self.params['b2']=b2
    ...
    def loss(self, X, y=None):
        ...
        h,cache1=Linear_ReLU.forward(X,W1,b1)
        scores,cache2=Linear.forward(h,W2,b2)
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        #######################################################################
        # Replace "pass" statement with your code
        num_h=len(hidden_dims)
        dimlist=[input_dim]+hidden_dims+[num_classes]
        for i in range(1,num_h+2):
            self.params[f'W{i}']=weight_scale*torch.randn(dimlist[i-1],dimlist[i],dtype=dtype,device=device)
            self.params[f'b{i}']=torch.zeros(dimlist[i],dtype=dtype,device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        # Replace "pass" statement with your code
        cur=X
        cache=[123]   #后面i是从1开始遍历的。所以cache的0号元素随便填一个占位
        # self.num_layers = 1 + len(hidden_dims)
        for i in range(1,self.num_layers+1):
            ...
            if not i==self.num_layers: 
                output,new_cache_1=Linear_ReLU.forward(cur,W,b)
                new_cache_2=[]
                if self.use_dropout:
                    output,new_cache_2=Dropout.forward(output,self.dropout_param)
                new_cache=[new_cache_1,new_cache_2]       

            else:
                output,new_cache=Linear.forward(cur,W,b)   #最后一层没有ReLU，要单独考虑
        ...
        for i in range(self.num_layers,0,-1):
            if i==self.num_layers:   #最后一层没有ReLU，要单独考虑
                dx,dW,db=Linear.backward(dout,cache[i])
            else:
                if self.use_dropout:
                    cache_2=cache[i][1]
                    dout=Dropout.backward(dout,cache_2)
                cache_1=cache[i][0]     
                dx,dW,db=Linear_ReLU.backward(dout,cache_1)
...
def get_five_layer_network_params():
    ...
    # model achieves 100% training accuracy within 20 epochs.      #
    ################################################################
    learning_rate = 0.15  # Experiment with this!
...
def adam(w, dw, config=None):
    ...
    m_nobias=m/(1-beta1**t)
    v_nobias=v/(1-beta2**t)
    w-=lr*m_nobias/(v_nobias.sqrt()+epsilon)
    next_w=w
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            out=x*mask/p   
            #vanilla dropout是train的时候out=x*mask，test的时候out=x*p
            #invert dropout是train的时候out=x*mask/p，test的时候out=x
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
        elif mode == 'test':
