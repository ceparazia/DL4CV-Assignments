...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        N,C,H,W=x.shape
        F,C,HH,WW=w.shape
        out_H=1+(H+2*pad-HH)//stride   #这里是整除，不要写成浮点数除法
        out_W=1+(W+2*pad-WW)//stride

        out=torch.zeros(N,F,out_H,out_W,dtype=x.dtype,device=x.device)  #(N,F,out_H,out_W)
        for batch in range(N):
            for filter in range(F):
                for i in range(out_H):
                    for j in range(out_W):
                        conv_weight=w[filter]  #(C,HH,WW)
                        h_start=i*stride
                        w_start=j*stride
                        x_slice=x_padded[batch,:,h_start:h_start+HH,w_start:w_start+WW]  #(C,HH,WW)
                        out[batch,filter,i,j]=(x_slice*conv_weight).sum()+b[filter]
    ...
    def backward(dout, cache):
        ...
        #dout是(N,F,H',W')
        dx_padded=torch.nn.functional.pad(dx,(pad,pad,pad,pad),"constant",0)


        for filter in range(F):
            db[filter]=torch.sum(dout[:,filter,:,:])
            ...
            for filter in range(F):
                for i in range(out_H):
                    for j in range(out_W):
                        ...
                        res=dout[batch,filter,i,j]
                        dx_padded[batch,:,h_start:h_start+HH,w_start:w_start+WW]+=res*w[filter]
                        dw[filter]+=res*x_padded[batch,:,h_start:h_start+HH,w_start:w_start+WW]
                        #梯度是累积的，这两行都是+=而不是=
                        #不需要除以N，因为传过来的上游梯度已经是除以N的了
        if pad>0:
    ...
    def forward(x, pool_param):
        ...
        for batch in range(N):
            for filter in range(C):
                for i in range(out_H):
                    for j in range(out_W):
                        out[batch,filter,i,j]=x[batch,filter,i*stride:i*stride+HH,j*stride:j*stride+WW].max()
    ...
    def backward(dout, cache):
        ...
        for batch in range(N):
            for filter in range(C):
                for i in range(out_H):
                    for j in range(out_W):
                        window=x[batch,filter,i*stride:i*stride+HH,j*stride:j*stride+WW]
                        mask=(window==window.max()).float()
                        dx[batch,filter,i*stride:i*stride+HH,j*stride:j*stride+WW]+=mask*dout[batch,filter,i,j]        
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        #          device='cpu')
        C,H,W=input_dims
        W1=weight_scale*torch.randn(num_filters,C,filter_size,filter_size,
        ...
        A_H=H//2
        A_W=W//2  
        #pad和stride已经保证conv前后长宽不变。
        #池化选取2*2的池子、步长是2，所以pool前后长宽都要//2
        D=num_filters*A_H*A_W

        #D=3*32*32=3072
        W2=weight_scale*torch.randn(D,hidden_dim,dtype=dtype,device=device)
    ...
    def loss(self, X, y=None):
        ...
        A,crp_cache=Conv_ReLU_Pool.forward(X,W1,b1,conv_param,pool_param)
        A_flatten=A.reshape(N,-1)

        Hidden,lr_cache=Linear_ReLU.forward(A_flatten,W2,b2)
        scores,l_cache=Linear.forward(Hidden,W3,b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        reg=self.reg
        loss+=reg*((W1*W1).sum()+(W2*W2).sum()+(W3*W3).sum())

        dHidden,dW3,db3=Linear.backward(dscores,l_cache)
        dW3+=2*reg*W3

        dA_flatten,dW2,db2=Linear_ReLU.backward(dHidden,lr_cache)
        dW2+=2*reg*W2

        dA=dA_flatten.reshape(A.shape)
        _,dW1,db1=Conv_ReLU_Pool.backward(dA,crp_cache)
        dW1+=2*reg*W1
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        C,H,W=input_dims
        F=[C]+num_filters
        #把X看作第0层隐藏层，则F[k]是第k层隐藏层的信道数
        #X是(N,C,H,W),则第k层隐藏层Hk的形状是(N,F[k],H',W')
        #每次经过pool则H和W减半，不经过pool则保持不变

        num_conv=len(num_filters)
        #num_conv次卷积+最终一次FC，一共要(num_conv+1)组参数
    ...
    def loss(self, X, y=None):
        ...
        #最后是FC层          
        FC_W=self.params[f'W{num_conv+1}']
        FC_b=self.params[f'b{num_conv+1}']
        scores,fc_cache=Linear.forward(out,FC_W,FC_b)
        ...
        loss,dscores=softmax_loss(scores,y)
        FC_W=self.params[f'W{num_conv+1}']
        loss+=reg*(FC_W*FC_W).sum()
        dFC_input,dFC_W,dFC_b=Linear.backward(dscores,caches[-1])
        dFC_W+=2*reg*FC_W
        grads[f'W{num_conv+1}']=dFC_W
        grads[f'b{num_conv+1}']=dFC_b
